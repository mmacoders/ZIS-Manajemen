<template>
  <div class="p-6">
    <div class="mb-6">
      ...
      <p class="text-gray-600">Kelola surat masuk</p>
    </div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <input 
          ...
          class="form-input"
          @keyup.enter="fetchIncomingLetters"
        />
        <button 
          @click="fetchIncomingLetters"
    ...
    <div class="card mb-6">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div>
          ...
          <select v-model="filters.program_id" class="form-select" @change="fetchIncomingLetters">
        ...
        <div>
          ...
          <select v-model="filters.status" class="form-select" @change="fetchIncomingLetters">
        ...
        <div>
          ...
          <input v-model="filters.start_date" type="date" class="form-input" @change="fetchIncomingLetters" />
        </div>
        
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Tanggal Selesai</label>
          <input v-model="filters.end_date" type="date" class="form-input" @change="fetchIncomingLetters" />
        </div>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-else-if="incomingLetters.length === 0">
            ...
            <tr v-else v-for="letter in incomingLetters" :key="letter.id" class="hover:bg-gray-50">
              <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ letter.nomor_agenda }}</td>
              <td class="px-6 py-4 text-sm text-gray-900">{{ formatDate(letter.tanggal) }}</td>
              <td class="px-6 py-4 text-sm text-gray-900">{{ letter.asal_surat }}</td>
              ...
              <td class="px-6 py-4 text-sm font-medium space-x-2">
                ...
                <button 
                  @click="deleteIncomingLetter(letter.id)"
      ...
      <div class="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto" @click.stop>
        <div class="p-6">
          <h3 class="text-lg font-bold text-gray-900 mb-6">
            {{ editingLetter ? 'Edit Surat Masuk' : 'Tambah Surat Masuk' }}
          </h3>
          
          <form @submit.prevent="saveIncomingLetter">
            <div class="space-y-4">
...
// Methods
const fetchIncomingLetters = async (page = 1) => {
  try {
    loading.value = true
    const params: any = {
      page,
    ...
    const response = await axios.get('/api/incoming-letters', { params })
    
    if (response.data.success) {
      incomingLetters.value = response.data.data.data || []
      pagination.value = {
      ...
      incomingLetters.value = response.data.data || []
    }
  } catch (error) {
...
const saveIncomingLetter = async () => {
  try {
    ...
    if (response.data.success) {
      alert(editingLetter.value ? 'Surat masuk berhasil diperbarui' : 'Surat masuk berhasil ditambahkan')
      closeModal()
      fetchIncomingLetters(pagination.value.current_page)
    } else {
      alert('Terjadi kesalahan: ' + (response.data.message || 'Unknown error'))
    }
  } catch (error: any) {
    console.error('Error saving incoming letter:', error)
...
const deleteIncomingLetter = async (id: number) => {
  if (confirm('Apakah Anda yakin ingin menghapus surat masuk ini?')) {
    try {
      const response = await axios.delete(`/api/incoming-letters/${id}`)
      if (response.data.success) {
